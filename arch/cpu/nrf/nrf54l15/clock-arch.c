...
schedule_next_tick(void)
{
   ...
   * REQUIRED, not just defensive. nrfx_grtc_syscounter_cc_absolute_set()
   * calls cc_channel_prepare() (which disables CCEN) and then writes only
   * CCL/CCH via nrf_grtc_sys_counter_cc_set() — it never re-enables CCEN.
   * Without the explicit write below the channel stays disabled and no
   * tick ever fires. Do not remove it as redundant.
   */
